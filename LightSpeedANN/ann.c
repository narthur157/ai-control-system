...
// Layer 2: 1 nodes, linear activation
...
#define L2_DEL (mem+56)
#define L2_N0_WEIGHTS (mem+60)
#define OUT_TMP (mem+72)

float *allocate_ann() {
    return (float *)_mm_malloc(76 * sizeof(float), 16);
}
...
__attribute__((noinline)) float *forward_L2_ann(float *mem) {
    int i, k;
    *(L2_VAL+0) = dotprod_8(L2_N0_WEIGHTS, L1_VAL) + *(L2_N0_WEIGHTS+8);
    return L2_VAL;
}
...
__attribute__((noinline)) void backward_ann(float *desired, float *mem, float lr) {
    ...
    subtract_1(OUT_TMP, L2_VAL, L2_DEL);

    /* Layer deltas 2 */
    memory_clear_8(L1_DEL);
    sum_scaled_8(L2_N0_WEIGHTS, L1_DEL, *(L2_DEL+0));
    ...
    /* Adjust weights */
    odel = *(L2_DEL+0) * lr;    *(L2_N0_WEIGHTS+8) += odel;    sum_scaled_8(L1_VAL, L2_N0_WEIGHTS, odel);
}

int layer_values_ann[3] = {0, 4, 52};

#else /* HEADER FOLLOWS */

float *allocate_ann();
void free_ann(float *mem);
#define MEM_SIZE_ann ( 76 * sizeof(float) )
